...
**RELAY** is a self-hosted, browser-based voice / video / chat caller. Two or more people open the site, each gets a 6-digit number, and they dial each other to start a peer-to-peer WebRTC mesh call (up to 6 participants). No accounts, no installs, no scheduling. Hosted on **Manus** at `https://relaychat-lduywq6l.manus.space` and (intended) on a custom domain after binding.
...
| Vitest suite (signaling protocol + filter registry + auth.logout) | 17/17 passing | `server/relay.test.ts`, `client/src/lib/mediaPipeline.test.ts`, `server/auth.logout.test.ts` |
