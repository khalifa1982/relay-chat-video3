...
describe("v2.105.11 — an unroutable token must not suppress the email fallback", () => {
  ...
  it("nothing sends an apns token to a transport", () => {
    // The fan-out filters on the two native kinds explicitly, so apns is excluded by
    ...
    expect(codeOnly(wp)).not.toMatch(/s\.kind === "apns"/);
  });
});
